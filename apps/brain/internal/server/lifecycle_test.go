...
func TestRunFailsFastWhenAPortIsTaken(t *testing.T) {
	...
	go func() { done <- New(cfg, discardLogger(), deps(healthyDB())).Run(t.Context()) }()
...
func TestRunWithAlreadyCancelledContext(t *testing.T) {
	...
	go func() { done <- New(cfg, discardLogger(), deps(healthyDB())).Run(ctx) }()
...
func TestRunLogsBothBindAddresses(t *testing.T) {
	...
	go func() { done <- New(cfg, logger, deps(healthyDB())).Run(ctx) }()
	if err := waitFor(t, done); err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestShutdownBeforeRun(t *testing.T) {
	...
	if err := New(testConfig(), discardLogger(), deps(healthyDB())).shutdown(); err != nil {
...
func TestListenMapsServerClosedToNil(t *testing.T) {
	...
	s := newHTTPServer(freeAddr(t), New(testConfig(), discardLogger(), deps(healthyDB())).apiHandler())
...
func TestListenReturnsBindError(t *testing.T) {
	...
	if err := listen(newHTTPServer(occupiedAddr(t), New(testConfig(), discardLogger(), deps(healthyDB())).apiHandler())); err == nil {
